...
export default function AnimalsView() {
  ...
  // --- HANDLERS & LOGIC (and computed/memo) ---
  ...
  const handleActionSearch = useCallback(async () => {
    ...
    try {
      if (!actionSearch || actionSearch.trim() === '') {
        setActionError(
          'Por favor, ingresa un identificador para poder buscar.',
        );
        return;
      }
      ...
      const found = data.find(
        (a) =>
          ...
          actionSearch.trim().toLowerCase(),
      );

      if (!found) {
        setActionError(
          `No se encontró ningún animal con el Identificador exacto: "${actionSearch}".`,
        );
        return;
      }
  ...
  const calculateGains_Internal = (a) => {
    let g4m = null,
      gmD = null,
      gMens = null;
    if (a.weight_4m && a.birth_weight)
      g4m = ((a.weight_4m - a.birth_weight) / 120).toFixed(2);
    if (a.weaning_weight && a.birth_weight)
      gmD = ((a.weaning_weight - a.birth_weight) / 210).toFixed(2);
    if (a.current_weight && a.birth_weight && a.birth_date) {
      const daysAlive = Math.max(
        1,
        (new Date() - new Date(a.birth_date)) / (1000 * 60 * 60 * 24),
      );
    ...
    try {
      ...
      const exportData = data.map((a) => {
        return {
          ...
          'FECHA DE NACIMIENTO': a.birth_date
            ? a.birth_date.split('T')[0].split('-').reverse().join('/')
            : '',
          'PENULTIMO PARTO': a.second_last_calving_date
            ? a.second_last_calving_date
                ...
                .join('/')
            : '',
          'ULTIMO PARTO': a.last_calving_date
          ...
          'FECHA DE COMPRA': a.purchase_date
            ? a.purchase_date.split('T')[0].split('-').reverse().join('/')
            : '',
          'FECHA DE VENTA': a.sale_date
            ? a.sale_date.split('T')[0].split('-').reverse().join('/')
            : '',
          'FECHA DE MUERTE': a.death_date
            ? a.death_date.split('T')[0].split('-').reverse().join('/')
            : '',
          GRADO: a.grado || '',
        ...
        try {
          ...
          for (const rawRow of data) {
            ...
            try {
              ...
              if (
                ...
                console.log(
                  'Fila saltada (sin identificador o encabezado):',
                  identifier,
                );
                continue;
              }
              ...
              if (rType.includes('TORO')) normalizedType = 'TORO';
                ...
                rType.includes('DESMADRE_HEMBRA') ||
                rType.includes('DESMADRE HEMBRA')
              )
                normalizedType = 'DESMADRE_HEMBRA';
              else if (
                rType.includes('DESMADRE_MACHO') ||
                rType.includes('DESMADRE MACHO') ||
                rType.includes('DESMADRE')
              )
                normalizedType = 'DESMADRE_MACHO';
              ...
              if (!sex)
                sex =
                  normalizedType === 'TORO' || normalizedType === 'CHIVO'
                    ? 'M'
                    : 'H';
              else sex = sex.startsWith('M') ? 'M' : 'H';

              const loteVal = String(
                getVal(row, ['L1', 'LOTE', 'L']) || 'GENERAL',
              )
                .toUpperCase()
                .substring(0, 95);
              ...
              const payload = {
                ...
                second_last_calving_date: parseDate(
                  getVal(row, ['PENULTIMOPARTO', 'FECHAPP', 'PENULTIMOP']),
                ),
              };
              ...
              const saleDate = parseDate(
                getVal(row, ['FECHADEVENTA', 'FECHAVENTA', 'VENTA']),
              );
              if (saleDate) {
              ...
              const deathDate = parseDate(
                getVal(row, ['FECHADEMUERT', 'FECHADEMUERTE', 'MUERTE']),
              );
              if (deathDate) {
              ...
              if (
                rawMother &&
                !['-', '0', 'N/A', 'NONE', 'NULL'].includes(
                  rawMother.toUpperCase(),
                )
              ) {
              ...
              failCount++;
              errors.push(
                `${getVal(rawRow, ['NVACA', 'ANIMAL']) || 'Fila'}: ${err.message}`,
              );
            }
          }
          ...
          if (pendingRelations.length > 0) {
            try {
              ...
              const animalsList = dbDataRes.data.data || dbDataRes.data;
              animalsList.forEach((a) => {
                if (!idMap.has(a.identifier)) idMap.set(a.identifier, a.id);
              });
                  ...
                  try {
                    const mRes = await axios.post(
                      '/animals',
                      {
                        ...
                        status: 'MUERTO',
                        origin: 'NACIMIENTO',
                        observations: 'Referencia historica (Madre muerta)',
                      },
                    );
          ...
          if (successCount === 0 && failCount === 0) {
            CustomAlert.info(
              'Aviso',
              "No se importó ningún animal. Verifica que los nombres de las columnas en tu Excel coincidan con lo esperado (ej: 'N. Vaca', 'Clasificación', 'Lote').",
            );
          } else if (failCount > 0) {
            CustomAlert.info(
              'Aviso',
              `Importación finalizada: ${successCount} exitosos, ${failCount} fallidos.`,
            );
          } else {
            CustomAlert.info(
              'Aviso',
              `¡Éxito! ${successCount} animales importados correctamente.`,
            );
          }
  ...
  const renderedRows = useMemo(() => {
    ...
    const exactMatches = animals.filter(
      (a) => (a.identifier || '').toLowerCase().trim() === query,
    );
    ...
    exactMatches.forEach((targetAnimal) => {
      ...
      const children = animals.filter(
        (c) =>
          c.mother_id == targetAnimal.id ||
          (c.mother && c.mother.id == targetAnimal.id) ||
          (c.mother &&
            (c.mother.identifier || '').toLowerCase().trim() ===
              (targetAnimal.identifier || '').toLowerCase().trim()),
      );
      ...
      const motherObj =
        targetAnimal.mother ||
        animals.find((m) => m.id == targetAnimal.mother_id);
    ...
    if (exactMatches.length === 0) {
      const orphanChildren = animals.filter(
        (c) =>
          (c.mother &&
            (c.mother.identifier || '').toLowerCase().trim() === query) ||
          (c.motherIdentifier &&
            c.motherIdentifier.toLowerCase().trim() === query),
      );
    ...
    <div className="fade-in">
      ...
      >
        <div>
          ...
          <p style={{ color: 'var(--text-muted)' }}>
            HISTORIAL COMPLETO DE ANIMALES
          </p>
        </div>
        ...
        {!isLoading && totalRecords > 0 && (
          ...
          >
            <div style={{ color: 'var(--text-muted)', fontSize: '14px' }}>
              Página {page} de {totalPages} ({totalRecords} animales)
            </div>
          ...
          >
            <h2
              style={{
                fontSize: '1.5rem',
                marginBottom: '8px',
                color: actionDialog.mode === 'DELETE' ? '#ef4444' : '#10b981',
              }}
            >
              {actionDialog.mode === 'EDIT'
                ? 'Editar Registro'
                : 'Eliminar Registro'}
            </h2>
            <div className="form-group">
              <input
                type="text"
                className="input-field"
                placeholder="Identificador (ej: 1/26)"
                value={actionSearch}
                onChange={(e) => {
            ...
            >
              ...
              <button
                ...
                style={{
                  background:
                    actionDialog.mode === 'DELETE' ? '#ef4444' : '#10b981',
                }}
                onClick={handleActionSearch}
              >
                Buscar y {actionDialog.mode === 'EDIT' ? 'Editar' : 'Eliminar'}
              </button>
            </div>
          </div>
        </div>
      ...
      {deleteConfirm.isOpen && (
        <div className="modal-overlay fade-in">
          <div
            className="premium-card modal-content"
            style={{ maxWidth: '400px', borderTop: '4px solid #ef4444' }}
          >
            <h2
              style={{
                fontSize: '1.5rem',
            ...
            <p style={{ color: 'var(--text-muted)', marginBottom: '24px' }}>
              Estás a punto de eliminar permanentemente al animal identificador:{' '}
      ...
      {resetConfirm && (
        <div className="modal-overlay fade-in">
          <div
            className="premium-card modal-content"
            style={{ maxWidth: '400px', borderTop: '4px solid #f97316' }}
          >
            <h2
              style={{
                fontSize: '1.5rem',
