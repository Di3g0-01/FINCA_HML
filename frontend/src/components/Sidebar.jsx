...
import { LayoutDashboard, LogOut, FileText, ShoppingCart, DollarSign, AlertCircle, Users, WalletCards, X, Activity, ChevronDown, ChevronRight, Briefcase, Beef } from 'lucide-react';
import './Sidebar.css';
export default function Sidebar({ isSidebarOpen, toggleSidebar, isAdmin, location, handleLogout }) {
  ...
  const renderNavLink = (item, isSubItem = false) => (
    <Link 
      ...
      style={{ 
        ...
        borderRadius: '8px', 
        color: location.pathname === item.to || (item.to !== '/dashboard' && location.pathname.includes(item.to)) ? (item.color || 'var(--primary-color)') : 'var(--text-muted)', 
        background: location.pathname === item.to || (item.to !== '/dashboard' && location.pathname.includes(item.to)) ? (item.color ? `${item.color}1a` : 'rgba(76, 175, 80, 0.1)') : 'transparent', 
        textDecoration: 'none', 
        transition: 'all 0.2s', 
        fontSize: '14px',
        whiteSpace: 'nowrap',
  ...
  return (
    <aside className={`sidebar ${isSidebarOpen ? 'sidebar-expanded' : 'sidebar-collapsed'}`} style={{ 
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: isSidebarOpen ? 'space-between' : 'center', marginBottom: '40px' }}>
        <div 
          ...
          tabIndex={0}
        >
          <img src="/logo.png" alt="Logo Finca HM" style={{ width: '40px', height: '40px', objectFit: 'contain', background: '#fff', borderRadius: '50%', minWidth: '40px' }} />
          {isSidebarOpen && <h3 style={{ fontSize: '1.2rem', fontWeight: '600', whiteSpace: 'nowrap' }}>Finca HM</h3>}
        </div>
        <button className="mobile-only" onClick={toggleSidebar} aria-label="Cerrar menú" style={{ background: 'transparent', color: 'var(--text-muted)' }}><X size={24} /></button>
      </div>

      <nav style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '6px', overflowY: 'auto', paddingRight: '4px', overflowX: 'hidden' }} className="custom-scrollbar">
        {isAdmin ? (
          <>
            ...
            <div style={{ marginTop: '8px' }}>
              <button onClick={() => { if (!isSidebarOpen) toggleSidebar(); else toggleMenu('ganaderia'); }} aria-expanded={expandedMenus.ganaderia} aria-label="Menú Ganadería" style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: isSidebarOpen ? 'space-between' : 'center', background: 'transparent', border: 'none', color: 'white', padding: '10px 12px', cursor: 'pointer', borderRadius: '8px', ':hover': { background: 'rgba(255,255,255,0.05)' } }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', fontWeight: '500', fontSize: '14px' }}>
                  <Beef size={20} color="#4CAF50" style={{ minWidth: '20px' }} /> 
                  {isSidebarOpen && <span>Ganadería</span>}
                </div>
                {isSidebarOpen && (expandedMenus.ganaderia ? <ChevronDown size={16} /> : <ChevronRight size={16} />)}
              </button>
            ...
            <div style={{ marginTop: '8px' }}>
              <button onClick={() => { if (!isSidebarOpen) toggleSidebar(); else toggleMenu('administracion'); }} aria-expanded={expandedMenus.administracion} aria-label="Menú Administración" style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: isSidebarOpen ? 'space-between' : 'center', background: 'transparent', border: 'none', color: 'white', padding: '10px 12px', cursor: 'pointer', borderRadius: '8px', ':hover': { background: 'rgba(255,255,255,0.05)' } }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', fontWeight: '500', fontSize: '14px' }}>
                  <Briefcase size={20} color="#9C27B0" style={{ minWidth: '20px' }} /> 
                  {isSidebarOpen && <span>Administración</span>}
                </div>
                {isSidebarOpen && (expandedMenus.administracion ? <ChevronDown size={16} /> : <ChevronRight size={16} />)}
              </button>
      ...
      <button onClick={handleLogout} aria-label="Cerrar sesión" style={{ display: 'flex', alignItems: 'center', justifyContent: isSidebarOpen ? 'flex-start' : 'center', gap: '12px', padding: '12px', borderRadius: '8px', color: 'var(--danger-color)', background: 'transparent', width: '100%', textAlign: 'left', border: '1px solid transparent', marginTop: 'auto' }} className="logout-btn">
