...
export default function PayrollView() {
  ...
  const handleSaveToDB = async () => {
    try {
      const details = {
        ...
        temporales: temporales.map((w) => ({
          name: w.name,
          ...calculateTemporalRow(w),
        })),
      };
      ...
      CustomAlert.info(
        'Aviso',
        '¡Nómina guardada exitosamente en el historial histórico!',
      );
    } catch (e) {
      CustomAlert.error('Error', 'Error al guardar la nómina.');
      throw e;
    }
  };
  ...
  const exportPDF = async () => {
    const result = await Swal.fire({
      ...
      icon: 'question',
      title: 'Exportar PDF',
      text: '¿Desea Guardar en BDD y Exportar, o Solo Exportar?',
      showCancelButton: true,
      showDenyButton: true,
      confirmButtonText: 'Guardar y Exportar',
      denyButtonText: 'Solo Exportar',
    ...
    if (result.isConfirmed) {
      try {
        ...
        return; // Don't export if save failed
      }
    }
    ...
    <div className="fade-in">
      ...
      >
        <label className="form-label" style={{ color: 'var(--primary-color)' }}>
          Nombre / Período del Registro a Generar
        </label>
        <input
          type="text"
      ...
      {/* --- TABLA DE FIJOS --- */}
      <h2
        style={{ fontSize: '1.5rem', marginBottom: '16px', color: '#4CAF50' }}
      >
        1. Trabajadores Fijos (Descuento IGSS)
      </h2>
      <div
      ...
      >
        ...
        >
          <thead>
            ...
            >
              <th
                style={{ padding: '16px', color: 'white', fontWeight: 'bold' }}
              >
                Nombre
              </th>
              <th
          ...
          <tbody>
            {fijos.length === 0 && (
              <tr>
                <td colSpan="7" style={{ padding: '16px' }}>
                  No hay trabajadores fijos activos.
                </td>
              </tr>
            )}
            {fijos.map((w) => {
                ...
                >
                  ...
                  <td style={{ padding: '16px' }}>
                    <input
                      ...
                      onChange={(e) =>
                        handleInputChange(w.id, 'extras', e.target.value)
                      }
                    />
                  </td>
                  <td style={{ padding: '16px' }}>
            ...
            >
              <td colSpan="6" style={{ padding: '16px', textAlign: 'right' }}>
                TOTAL NÓMINA FIJA (Q):
              </td>
              <td
                style={{
              ...
              >
                ...
                {fijos
                  .reduce(
                    (sum, w) => sum + calculateFijoRow(w).totalRecibido,
                    0,
                  )
                  .toFixed(2)}
              </td>
      ...
      {/* --- TABLA DE TEMPORALES --- */}
      <h2
        style={{ fontSize: '1.5rem', marginBottom: '16px', color: '#FF9800' }}
      >
        2. Trabajadores Temporales (Cobro por Día)
      </h2>
      <div
        className="premium-card"
        style={{ marginBottom: '40px', overflowX: 'auto' }}
          ...
          <tbody>
            {temporales.length === 0 && (
              <tr>
                <td colSpan="6" style={{ padding: '16px' }}>
                  No hay trabajadores temporales activos.
                </td>
              </tr>
            )}
            {temporales.map((w) => {
                ...
                >
                  ...
                  <td style={{ padding: '16px' }}>
                    <input
                      ...
                      onChange={(e) =>
                        handleInputChange(w.id, 'days', e.target.value)
                      }
                    />
                  </td>
                  <td style={{ padding: '16px', color: 'var(--text-muted)' }}>
            ...
            >
              ...
              >
                ...
                {temporales
                  .reduce(
                    (sum, w) => sum + calculateTemporalRow(w).totalRecibido,
                    0,
                  )
                  .toFixed(2)}
              </td>
