...
export default function LogsView() {
  ...
  const handleDeleteLog = async (id) => {
    const result = await CustomAlert.confirm(
      '¿Estás seguro de eliminar este registro de actividad permanentemente?',
    );
  ...
  const handleClearAllLogs = async () => {
    const result = await CustomAlert.confirm(
      ...
      'Esta acción borrará TODA la bitácora de actividad permanentemente y no se puede deshacer.',
    );
    if (!result.isConfirmed) return;
    try {
  ...
  const checkAndCleanupOldLogs = useCallback(async () => {
    try {
      ...
      const oldLogs = allLogs.filter(
        (log) => new Date(log.created_at) < sixMonthsAgo,
      );

      if (oldLogs.length > 0) {
        CustomAlert.info(
          'Aviso',
          `SISTEMA: Se han detectado ${oldLogs.length} registros con más de 6 meses de antigüedad. 
      ...
      >
        <div>
          ...
          <p style={{ color: 'var(--text-muted)' }}>
            HISTORIAL COMPLETO DE MOVIMIENTOS Y CAMBIOS EN EL SISTEMA
          </p>
        </div>
          ...
          >
            ...
            >
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <span style={{ fontWeight: 'bold' }}>
                  {selectedLog.animal_identifier || 'N/A'}
                </span>
              </div>
              <div
                style={{
              ...
              >
                ...
                <span>
                  {new Date(selectedLog.created_at).toLocaleString('es-GT')}
                </span>
              </div>
            </div>
