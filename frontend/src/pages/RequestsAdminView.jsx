...
export default function RequestsAdminView() {
  ...
  const handleApprove = async (id) => {
    const confirm = await CustomAlert.confirm(
      'Aprobar Solicitud',
      '¿Estás seguro de que deseas aprobar esta solicitud y reflejarla en el inventario?',
    );
  ...
  const handleReject = async (id) => {
    const confirm = await CustomAlert.confirm(
      'Rechazar Solicitud',
      '¿Deseas rechazar y archivar esta solicitud?',
    );
    if (confirm.isConfirmed) {
      try {
  ...
  const handleClearAll = async () => {
    const confirm = await CustomAlert.confirm(
      'Limpiar Historial',
      '¿Estás seguro de que deseas eliminar permanentemente todo el historial de solicitudes? Esta acción no se puede deshacer.',
    );
    if (confirm.isConfirmed) {
      try {
                  ...
                  requests.map((req) => (
                    ...
                    >
                          ...
                          style={{
                            ...
                            background:
                              req.type === 'NACIMIENTO'
                                ...
                                : 'rgba(255, 152, 0, 0.1)',
                            color:
                              req.type === 'NACIMIENTO' ? '#4CAF50' : '#FF9800',
                          }}
                        >
                          {req.type}
                        </span>
                      ...
                      <td style={{ padding: '16px' }}>
                        {req.type === 'NACIMIENTO'
                          ? `${req.payload.type} - Sexo: ${req.payload.sex}`
                          : `Muerte Animal ID: ${req.payload.animal_id} - ${req.payload.death_reason || 'Sin causa'}`}
                      </td>
                      <td
                        style={{ padding: '16px', color: 'var(--text-muted)' }}
                      >
                        {req.requester?.username || 'Desconocido'}
                      </td>
                      <td
                        style={{ padding: '16px', color: 'var(--text-muted)' }}
                      >
