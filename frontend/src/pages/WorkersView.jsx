    ...
    if (
      (
        await CustomAlert.confirm(
          `¿Estás seguro de eliminar a ${name}? Esto borrará su historial de la base de datos de manera definitiva.`,
        )
      ).isConfirmed
    ) {
      try {
                ...
                {workers.length === 0 ? (
                  ...
                  workers.map((w) => (
                    ...
                    >
                      ...
                      >
                        {w.salary
                          ? `Q ${w.salary.toFixed(2)} ${w.contract_type === 'TEMPORAL' ? '/hr' : '/mes'}`
                          : '-'}
                      </td>
                      <td
                        style={{
